...
import {TileFormat, MAP_TYPES, TILE_FORMATS} from '../api/maps-api-common';
...
> {
  ...
  getQueryUrl(tile: TileLoadProps, tilesetUrl: string) {
    ...
    if (type === MAP_TYPES.TABLE) {
      dataQuery = `select ${uniqueIdProperty}, ${columns.join(', ')} FROM ${data}`;
    } else if (type === MAP_TYPES.QUERY) {
      dataQuery = data;
    } else {
      throw new Error('Unsupported type for named areas', type);
    }
